...
const add = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (id && pass) { // id, pass가 참이면
        const bool = yield (0, DB_js_1.checkOverlap)('id', id); //아이디 중복확인
        if (bool) { // 중복되는 id가 없을 걍우
            const hash = (0, hash_1.default)(id, pass); // hash 만들기
            if (!mail)
                mail = `temp${Date.now()}@naver.com`; //메일이 없을경우 임시로 가입한 시간을 이용하여 채워놓기
            const obj = {
                id,
                hash,
                mail,
            ...
            const r = yield (0, DB_js_1.insertData)(obj); // userdata 추가
            if (r) { // 유저데이터가 정상적으로 추가되었을 시
                yield (0, DB_js_1.deleteData)({ mail }, 'tempmail'); // tempmail에 있는 이 메일 삭제
...
const del = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (id) { // id 가 있을 시
        const bool = yield (0, DB_js_1.checkOverlap)('id', id); //중복확인
        if (!bool) { // 중복확인이 거짓일 시
            const r = yield (0, DB_js_1.deleteData)({ id }); // 유저 정보 삭제
            if (r) { // 성공적으로 삭제 되었을 시
            ...
            res.json({ status: 'bad', reason: '삭제할 ID가 없습니다.' });
        }
    }
    else {
